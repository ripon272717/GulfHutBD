import { Navbar, Container, Badge, Image, Form, InputGroup, Nav, Button, Modal, Row, Col } from 'react-bootstrap';
  ...
  return (
    <header className='fixed-top shadow-sm'>
      <style>{`
        ...
        @media (max-width: 991px) { .pc-menu { display: none !important; } }
      `}</style>

      <Navbar variant='dark'>
        <Container fluid className='px-2'>
          <Navbar.Brand onClick={() => navigate('/')} style={{cursor:'pointer'}} className='d-flex align-items-center m-0'>
            <img src={logo} alt='logo' width='42'/>
            <div className='pc-menu ms-2' style={{fontSize: '9px', color: '#ffc107', fontWeight: 'bold'}}>GULF HUT</div>
          </Navbar.Brand>
          ...
          <div className='pc-menu'>
            <Nav className='gap-3 align-items-center'>
              ...
              <div className='position-relative ms-2' style={{color:'white', cursor:'pointer'}} onClick={() => navigate('/cart')}>
          ...
          <div className='d-flex align-items-center ms-auto'>
            {userInfo ? (
              <div className='d-flex align-items-center' onClick={() => setShowProfileModal(true)} style={{cursor:'pointer'}}>
                <div className='text-end me-2'>
                  <div style={{color:'white', fontSize:'12px', fontWeight:'bold'}}>{userInfo.name}</div>
            ...
            <FaBars className='text-white ms-3 d-lg-none' style={{fontSize: '22px', cursor: 'pointer'}} onClick={() => setShowSidebar(true)} />
          </div>
        </Container>
      </Navbar>
      <div className="notice-board"><marquee scrollamount="5">Fastest Delivery from Qatar to Bangladesh - Gulf Hut</marquee></div>
      ...
      <Modal show={showProfileModal} onHide={() => {setShowProfileModal(false); setIsEditMode(false);}} centered scrollable>
        <Modal.Body className='text-center p-4 position-relative'>
          <button type="button" onClick={() => {setShowProfileModal(false); setIsEditMode(false);}} style={{position:'absolute', right:'15px', top:'15px', border:'none', background:'#eee', borderRadius:'50%', width:'30px', height:'30px'}}><FaTimes /></button>
          ...
          <div className='bg-dark text-white p-3 rounded-4 mb-3 d-flex justify-content-between align-items-center'>
            <div className='text-start'><small className='opacity-75 text-warning'>Total Balance</small><h4 className='fw-bold mb-0'>QR {userInfo?.balance || 0}</h4></div>
          ...
          <Row className='g-2 mb-3'>
            <Col xs={6}>
               <Button variant='light' className='w-100 border py-2' onClick={() => {navigate(isAdmin ? '/admin/dashboard' : '/dashboard'); setShowProfileModal(false)}}>
                <FaChartLine className='text-warning d-block mx-auto mb-1'/> <b>Dashboard</b>
              </Button>
            </Col>
            <Col xs={6}>
               <Button variant='light' className='w-100 border py-2' onClick={() => {navigator.clipboard.writeText(inviteLink); toast.success('Link Copied!')}}>
          ...
          {isEditMode && (
            <Form className='text-start border-top pt-3'>
              <small className='fw-bold text-muted'>Name</small>
              <Form.Control className='edit-input shadow-none' value={name} onChange={(e) => setName(e.target.value)} />
          ...
          <Button variant='link' className='text-danger w-100 mt-2 text-decoration-none small fw-bold' onClick={logoutHandler}><FaSignOutAlt className='me-1'/> Logout Account</Button>
        </Modal.Body>
      </Modal>
